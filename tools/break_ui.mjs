...
const EXPECT = {
  ...
  "the plates leave short landscape no room under inflation": "leaves no card in any case",
...
const BREAKS = [
  ...
  ["the card floor token goes missing",
   "  --cw-floor: 32px;",
   "  /* --cw-floor: gone */"],
  ["the portrait block overrides the floor token",
   "    --cw: clamp(var(--cw-floor), min(var(--cw-height), var(--cw-width)), 168px);",
   "    --cw-floor: 36px;\n    --cw: clamp(var(--cw-floor), min(var(--cw-height), var(--cw-width)), 168px);"],
  ["the icon bar alone outgrows a short screen",
   "  --topbar: 48px;",
   "  --topbar: 400px;"],
  ["the plates leave short landscape no room under inflation",
   "</style>",
   "@media (orientation: landscape) and (max-height: 600px){ :root{ --plate-row: calc(var(--t-tiny) * 1.7); } }\n</style>"],
