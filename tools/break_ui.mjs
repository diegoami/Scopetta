...
const EXPECT = {
  ...
  "carte is a number with no working": "the working under carte lists",
  "the working under carte counts something else": "the working under carte counts",
...
const BREAKS = [
  ...
  ["carte is a number with no working",
   "    ...SEMI.map((nome, suit) =>\n    [nome,         bySuit(BASSO, suit), bySuit(ALTO, suit), null, true]),\n",
   ""],
  ["the working under carte counts something else",
   "  const bySuit = (w, s) => piles[w].filter(c => c.s === s).length;",
   "  const bySuit = (w, s) => piles[w].filter(c => c.s === s).length + (s === 2 ? 1 : 0);"],
